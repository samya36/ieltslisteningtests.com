document.addEventListener('DOMContentLoaded', function() {
    ...
    const audioConfig = {
      test3: {
        ...
        sections: [
          ...
          { id: 'section3', title: 'Section 3: Iceland Geology Trip', duration: '10:00' },
          { id: 'section4', title: 'Section 4: Tyre Recycling Australia', duration: '10:00' }
        ]
      }
    };
    window.enhancedAudioPlayer = new EnhancedAudioPlayer('test3', audioConfig);
  }

  // Generate questions list for answer sheet
  const allQuestions = [];
  for (let i = 1; i <= 10; i++) allQuestions.push({ id: i, section: 1, type: 'text', element: document.getElementById(`q${i}`) });
  for (let i = 11; i <= 16; i++) allQuestions.push({ id: i, section: 2, type: 'text', element: document.getElementById(`q${i}`) });
  for (let i = 17; i <= 20; i++) allQuestions.push({ id: i, section: 2, type: 'radio', name: `q${i}`, options: ['A','B','C'] });
  for (let i = 21; i <= 25; i++) allQuestions.push({ id: i, section: 3, type: 'radio', name: `q${i}`, options: ['A','B','C'] });
  for (let i = 26; i <= 30; i++) allQuestions.push({ id: i, section: 3, type: 'text', element: document.getElementById(`q${i}`) });
  for (let i = 31; i <= 40; i++) allQuestions.push({ id: i, section: 4, type: 'text', element: document.getElementById(`q${i}`) });

  if (typeof EnhancedAnswerSheet !== 'undefined') window.enhancedAnswerSheet = new EnhancedAnswerSheet(allQuestions);
  if (typeof EnhancedKeyboardNavigation !== 'undefined') window.keyboardNavigation = new EnhancedKeyboardNavigation();
  ...
  console.log('IELTS Test 3 ready. Press H for help.');
  ...
  window.submitTest = function(){
    ...
    const answerKey = {};
    if (typeof getTest3StandardAnswers === 'function'){
      const std = getTest3StandardAnswers();
